...
```{r}
chart(data = biometry, sommeil ~ fast_food) +
  geom_violin()
```
...
```{r_graphique_chrisstelle}

chart(data = biometry, taille ~ masse_corr %col=% genre ) +
geom_point(na.rm = TRUE) +
  labs(title = "Relation entre la taille et la masse",
       x = "masse (kg)",
       y = "taille (cm)")

```

**interprétation** les personnes les plus grandes tendent à avoir une masse plus élevée les hommes ont une masse plus élevée que les femmes pour une même taille les femmes sont concentrées dans la plage 50-80 Kg alors que les hommes couvrent un plus grand éventail de masses (jusqu'à 120 kg)

```{r_tableau_christelle}
biometry %>.%
