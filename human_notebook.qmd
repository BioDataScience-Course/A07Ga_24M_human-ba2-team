...
# Catégorisation de l'activité physique (nombre d'heures par semaine)
...
# Catégorisation de l'activité physique (nombre d'heures par semaine)
smutate(biometry, activite_physique_cat = case_when(
  activite_physique <= 2 ~ "Faible",
  activite_physique > 2 & activite_physique <= 5 ~ "Modéré",
  activite_physique > 5 ~ "Élevé"))
...
Les individus ayant une activité physique élevée ont un tour de poignet plus important, reflétant une ossature et une masse musculaire plus développées. Parmi eux, les carnivores affichent les poignets les plus larges, avec une médiane avoisinant les 17,5 cm. Cela pourrait s'expliquer par un apport plus élevé en protéines animales, qui favorisent le développement musculaire et osseux. Les omnivores suivent de près, avec une médiane légèrement inférieure, tandis que les herbivores, bien qu'actifs, ont les poignets les plus fins, même en comparaison avec les personnes inactives physiquement. Cela pourrait s'expliquer par un apport limité en protéines animales, qui influencerait leur masse osseuse (médiane autour de 15 cm).

Chez les personnes ayant une activité physique faible, le tour de poignet est plus petit (entre 15 et 17,5 cm), ce qui suggère une densité osseuse et une masse musculaire moindres. En revanche, dans ce groupe, les omnivores présentent une médiane plus élevée que les carnivores, tandis qu'aucun herbivore n'est représenté.
...
# Catégorisation de la masse corrigée (kg)
smutate(biometry, masse_corr_cat = case_when(
  masse_corr >= 0 & masse_corr <= 50 ~ "Faible",
...
-   Chez les hommes, la consommation moyenne de cigarettes augmente avec la masse : elle est nulle pour les individus à masse faible, passe à 2,73 pour ceux à masse modérée, puis à 3,41 pour ceux à masse élevée.

-   Chez les femmes, seules celles avec une masse modérée consomment du tabac, avec une moyenne de 12 cigarettes par semaine, ce qui est nettement plus élevé que chez les hommes dans la même catégorie.

-   L'écart-type chez les femmes à masse modérée (51,19) est très élevé, ce qui indique une forte variabilité : certaines consomment beaucoup, d'autres très peu. Chez les hommes, les écarts-types restent plus faibles (entre 6,86 et 11,67), reflétant une consommation plus homogène.
...
CALCUL DE LA OU DES VARIABLES REPRENANT L'INDICE SOUS FORME NUMERIQUE ( ET FACTEUR)
...
La variable dépendante est quantitative.
La variable explicative est qualitative binaire (deux groupes).
Les deux échantillons sont indépendants.
Les données suivent une distribution normale dans chaque groupe (vérifiée par un test de normalité ou supposée si n > 30).
...
La consommation d'alcool est un facteur comportemental fréquemment associé à une prise de poids, en raison de son apport énergétique élevé et de son influence sur les habitudes alimentaires. Il a été observé que les individus suivant un régime carnivore ont tendance à consommer davantage d'alcool que ceux ayant un régime herbivore. Cette analyse vise à déterminer si le type d'alimentation influence le nombre de verres d'alcool consommés par semaine, afin de mieux comprendre l'interaction entre les habitudes alimentaires et un facteur indirectement lié au risque d'obésité.
