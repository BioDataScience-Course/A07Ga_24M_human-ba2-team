...
-Dans la suite de notre travail, nous ferons un test statistique pour vérifier si cette différence est statistiquement significative.

HYPOTHESES
H0:différence significative de sommeil entre les hommes et les femmes 
H1: pas de différence de sommeil entre les hommes et les femmes 

```{r}
t.test(sommeil ~ genre, data = biometry, alternative = "two.sided", var.equal = TRUE)
```
Nous rejetons H0 au seuil de alpha 5%
Il n'y a pas de différence significative entre la durée de sommeil des hommes et des femmes (t = 0.24358, df = 142, p-value = 0.8079).
...
data$IC <- data$tour_taille / 100 / (0.109 * sqrt(data$masse / (data$taille / 100)))
summary(data$IC)
