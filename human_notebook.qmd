...
biometry <- biometry %>%
  mutate(
    sucre_cat = factor(case_when(
      sucre == "jamais" ~ "Faible",
      sucre %in% c("rarement") ~ "Modéré",
      sucre %in% c("souvent", "régulièrement") ~ "Élevé"
    )),
    ap_cat = factor(case_when(
      activite_physique < 150 ~ "Sédentaire",
      activite_physique < 300 ~ "Modéré",
      TRUE ~ "Élevé"
    )),
    sommeil_cat = factor(ifelse(sommeil < 6, "<6h", "≥6h"))
  )

# Création du tableau récapitulatif
table_recap <- biometry %>%
  select(risque_ic, sucre_cat, ap_cat, sommeil_cat) %>%
  pivot_longer(-risque_ic, names_to = "facteur", values_to = "categorie") %>%
  group_by(risque_ic, facteur, categorie) %>%
...
# Créer un tableau croisé entre risque_ic et chaque variable
table_risque_ic <- list()

# Variables à analyser
variables <- c("genre", "activite_physique", "alimentation", "alcool", 
               "intolerance_lactose", "sommeil", "intolerance_gluten", 
               "fast_food", "sucre", "fumeur", "cortisone", "eau", "age", "masse_corr", "whr")

# Créer les tableaux croisés pour chaque variable
for (var in variables) {
  table_risque_ic[[var]] <- table(biometry$risque_ic, biometry[[var]])
}

# Afficher les résultats
print(table_risque_ic)



```
```{r}
ggplot(table_recap, aes(x = facteur, y = pourcentage, fill = risque_ic)) +
  geom_bar(stat = "identity", position = "dodge") +
  ylab("Pourcentage d'individus") +
  xlab("Facteurs de risque") +
  ggtitle("Répartition des facteurs de risque selon l'Indice de Conicité") +
...
- Les personnes les plus jeunes ont tendance à avoir un IC modéré, tandis que les personnes plus âgées ont un IC élevé.
- Il y a une légère augmentation de la consommation d'alcool chez ceux qui ont un IC élevé 
- Les  personnes qui consomment plus de fast food ont modéré ( contre intuitif) 
- L'activité physique ne semble pas avoir d'impact sur l' IC
- avec un IC élevé consomment moins de fast food 
- le sommeil est un facteur potentiel car un IC élevé est associé à un sommeil plus court.

-   Dans la suite du travail nous effectuerons un test d'hypothèse pour vérifier si l'association entre le sommeil (facteur le plus lié) et l'indice de conicité est statistiquement significative.
...
<!--% Hypothèse nulle et alternative -->
 ...
 H0: Il n' y a pas d'association entre le sommeil et l'indice de conicité (ic)
 H1: Il y a une association significative entre le sommeil et l'indice de conicité (ic)
...
biometry_clean <- biometry %>% 
  filter(!is.na(sommeil_cat), !is.na(risque_ic))

# Créer la table de contingence
table_cont_sommeil <- table(biometry_clean$sommeil_cat, biometry_clean$risque_ic)

# Vérifier le tableau
print(table_cont_sommeil)
...
chisq.test(table_cont_sommeil)

#à completer dans l'interprétation : l'approximation donne un avis car il y a un grand écart entre sommeil <6 et sommeil ≥ 6
